...
from config import DELETE_LOCAL_AFTER_DRIVE, DRIVE_FOLDER, DRIVE_REMOTE, LOW_DISK_PERCENT
...
def archive_to_drive(video: Path, metadata_path: Path, thumbnail: Path | None, receipt: dict[str, Any]) -> str:
    if not drive_ready():
        raise RuntimeError(f"Rclone chưa có remote {DRIVE_REMOTE}. Chạy: rclone config")
    ...
    with tempfile.NamedTemporaryFile("w", suffix=".json", encoding="utf-8", delete=False) as handle:
        ...
        receipt_file = Path(handle.name)
    files.append(receipt_file)
    try:
        for path in files:
            result = subprocess.run(
                ["rclone", "copyto", str(path), f"{remote_dir}/{path.name}", "--checksum", "--retries", "5", "--low-level-retries", "10"],
                capture_output=True, text=True, check=False,
            )
            if result.returncode != 0:
                raise RuntimeError(result.stderr.strip() or "rclone thất bại")
        check = subprocess.run(["rclone", "lsf", remote_dir], capture_output=True, text=True, check=False)
        if check.returncode != 0 or video.name not in check.stdout.splitlines():
