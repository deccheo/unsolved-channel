...
def run() -> None:
    ...
    logging.info('Agent 2 V4 bắt đầu | hàng chờ=%s', len(cases))
    ...
    logging.info(
        'THỐNG KÊ AGENT 2 V4 | verified=%s | review=%s | retry=%s | failed=%s | lỗi=%s',
        stats['verified'], stats['review'], stats['retry'], stats['failed'], stats['error'],
    )
    logging.info('Agent 2 V4 hoàn thành')
