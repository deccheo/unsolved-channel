...
def run():
    ...
    logging.info(
        "Agent 7 TURBO V2 bắt đầu: %s hồ sơ | workers=%s | part_size=%s",
    ...
    for case in cases:
        ...
        logging.info(
            "Hồ sơ %s | READY liên tục=%s/%s | part đủ điều kiện=%s",
            title, len(prefix), total, len(parts),
        )

        if not parts:
            needed = min(PART_SIZE, total) if total else PART_SIZE
            logging.warning(
                "Chưa đủ cảnh liên tục để tạo part: %s | hiện=%s | cần=%s",
                title, len(prefix), needed,
            )
            continue
        ...
        # Khi toàn bộ cảnh liên tục đã READY, ghép các part thành final.
        ...
        if complete_story:
            try:
                ...
                if len(done_rows) == expected_parts and [int(r["part_no"]) for r in done_rows] == list(range(1, expected_parts + 1)):
                    ...
                    save_video(case["id"], str(final_path), final_duration, total, True)
                    stats["finals"] += 1
                    logging.info(
    ...
    elapsed = time.perf_counter() - started
    logging.info(
        "THỐNG KÊ AGENT 7 V2 | part mới=%s | part reuse=%s | clip mới=%s | "
    ...
    logging.info("Agent 7 TURBO V2 hoàn thành")
