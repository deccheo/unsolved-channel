...
def run():
    ...
    logging.info('Agent 2 V7 bắt đầu | hàng chờ=%s', len(cases))
    ...
    for index, case in enumerate(cases, 1):
        ...
        try:
            ...
            usable = [
                s for s in sources
                if len((s['extracted_text'] or '').strip()) >= MIN_TEXT_LENGTH
                and int(s['reliability_score'] or 0) >= 50
                and int(s['relevance_score'] or 0) >= 3
            ]
    ...
    logging.info('THỐNG KÊ AGENT 2 V7 | verified=%s | review=%s | retry=%s | failed=%s | lỗi=%s', stats['verified'], stats['review'], stats['retry'], stats['failed'], stats['error'])
    logging.info('Agent 2 V7 hoàn thành')
