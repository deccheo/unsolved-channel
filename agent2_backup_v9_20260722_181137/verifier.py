...
- enough_sources=true when there is at least one official source, OR one reputable news source with substantial case-specific detail, OR two independent secondary sources.
- A single source may support VERIFIED only when reliability is at least 50, relevance is at least 3, and the article contains concrete case facts.
- If evidence is plausible but not strong enough for VERIFIED, use NEEDS_REVIEW rather than REJECTED.
