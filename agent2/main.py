...
from database import (migrate,assert_schema,pending_cases,add_source,get_sources,mark_verified_guarded,
                      mark_review,schedule_retry,repair_orphan_verified)
...
def process_case(case,search_fn=search_sources,extract_fn=extract_text,verify_fn=verify):
    ...
    if not name:
        status,*_=schedule_retry(case['id'],'Case thiếu tên'); return status
    ...
    for source in found[:MAX_SOURCES_PER_CASE]:
        try:
            ...
            if add_source(case['id'],source): persisted+=1
    ...
    stored=get_sources(case['id'])
    usable=_usable_sources(stored)
    logging.info('Đối chiếu nguồn DB | tìm=%s | ghi/upsert=%s | tổng DB=%s | dùng được=%s',len(found),persisted,len(stored),len(usable))
    if len(usable)<MIN_USABLE_SOURCES:
        status,retry_count,next_retry=schedule_retry(case['id'],f'Chưa đủ nguồn persisted phù hợp: {len(usable)}/{MIN_USABLE_SOURCES}')
    ...
    if verified:
        count=mark_verified_guarded(case['id'],result,MIN_USABLE_SOURCES,MIN_TEXT_LENGTH,MIN_RELIABILITY,MIN_RELEVANCE)
        logging.info('VERIFIED | điểm=%s | nguồn persisted=%s',score,count); return 'VERIFIED'
...
def run():
    setup_logging(); migrate(); assert_schema()
    repaired=repair_orphan_verified(MIN_USABLE_SOURCES,MIN_TEXT_LENGTH,MIN_RELIABILITY,MIN_RELEVANCE)
    if repaired: logging.warning('Đã đưa %s case VERIFIED thiếu nguồn về hàng chờ: %s',len(repaired),','.join(str(x) for x in repaired))
    else: logging.info('Không có case VERIFIED thiếu nguồn cần sửa')
    cases=pending_cases(MAX_CASES_PER_RUN)
    logging.info('Agent 2 V%s bắt đầu | hàng chờ=%s',VERSION,len(cases)); stats={'VERIFIED':0,'NEEDS_REVIEW':0,'RETRY':0,'FAILED':0,'ERROR':0,'REPAIRED':len(repaired)}
