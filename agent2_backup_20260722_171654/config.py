...
CACHE_PATH = BASE_DIR / 'data' / 'agent2_search_cache.json'
...
USER_AGENT = os.getenv(
    'AGENT2_USER_AGENT',
    'Mozilla/5.0 (compatible; UnsolvedCaseResearchBot/5.0)',
)
MAX_CASES_PER_RUN = int(os.getenv('AGENT2_MAX_CASES', '5'))
MAX_SOURCES_PER_CASE = int(os.getenv('AGENT2_MAX_SOURCES', '8'))
MIN_USABLE_SOURCES = int(os.getenv('AGENT2_MIN_SOURCES', '2'))
MIN_VERIFIED_SCORE = int(os.getenv('AGENT2_MIN_SCORE', '40'))
MAX_RETRIES = int(os.getenv('AGENT2_MAX_RETRIES', '3'))
SEARCH_CACHE_HOURS = int(os.getenv('AGENT2_SEARCH_CACHE_HOURS', '168'))
BACKOFF_MINUTES = tuple(
    int(x.strip())
    for x in os.getenv('AGENT2_BACKOFF_MINUTES', '5,30,120').split(',')
    if x.strip()
)
