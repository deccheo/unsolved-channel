...
from config import (
    GOOGLE_CSE_API_KEY, GOOGLE_CSE_CX, LOG_PATH,
    MAX_CASES_PER_RUN, MAX_SOURCES_PER_CASE,
    ...
    for index, case in enumerate(cases, 1):
        ...
        try:
            ...
            usable = [
                ...
                if len((source['extracted_text'] or '').strip()) >= MIN_TEXT_LENGTH
            ]
            ...
            if verified:
                ...
                stats['review'] += 1
                logging.info(
                    'NEEDS_REVIEW | điểm=%s | khuyến nghị=%s | risk=%s',
            ...
            stats['error'] += 1
            logging.warning(
                '%s sau lỗi | retry=%s | lần kế=%s',
                status, retry_count, next_retry or '-',
            )

    logging.info(
        'THỐNG KÊ AGENT 2 V5 | verified=%s | review=%s | retry=%s | failed=%s | lỗi=%s',
        stats['verified'], stats['review'], stats['retry'], stats['failed'], stats['error'],
    )
    logging.info('Agent 2 V5 hoàn thành')
