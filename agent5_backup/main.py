import logging
import shutil
import re
import sys
import time

from config import (
    LOG_PATH,
    OUTPUT_DIR,
    MAX_CASES_PER_RUN,
    MAX_IMAGES_PER_RUN,
    RETRY_LIMIT,
    RETRY_DELAY_SECONDS,
)
from database import (
    migrate,
    pending_cases,
    pending_scenes,
    find_cached_image,
    mark_scene_cached,
    mark_scene_ready,
    mark_scene_retry,
    finalize_case,
)
from imagen import generate_image

def setup_logging():
    LOG_PATH.parent.mkdir(parents=True, exist_ok=True)
    logging.basicConfig(
        level=logging.INFO,
        format="%(asctime)s | %(levelname)s | %(message)s",
        handlers=[
            logging.FileHandler(LOG_PATH, encoding="utf-8"),
            logging.StreamHandler(sys.stdout),
        ],
    )

def slugify(text: str) -> str:
    text = text.lower()
    text = re.sub(r"[^a-z0-9]+", "-", text)
    return text.strip("-")[:80] or "case"

def run():
    setup_logging()
    migrate()

    cases = pending_cases(MAX_CASES_PER_RUN)
    logging.info("Agent 5 bắt đầu: %s hồ sơ", len(cases))

    remaining = MAX_IMAGES_PER_RUN

    scene_count = 0

    api_calls = 0

    success_count = 0

    error_count = 0

    retry_count = 0
    cache_hits = 0
    for case_index, case in enumerate(cases, 1):
        if remaining <= 0:
            break

        title = case["script_title"] or case["case_name"] or case["title"]
        folder = OUTPUT_DIR / f"{case['id']:04d}-{slugify(title)}"
        scenes = pending_scenes(case["id"], remaining)

        logging.info(
            "[%s/%s] Tạo ảnh: %s | %s cảnh trong lần chạy này",
            case_index,
            len(cases),
            title,
            len(scenes),
        )

        for scene in scenes:

            scene_count += 1
            output = folder / f"scene_{scene['scene_no']:03d}.jpg"

            cached = find_cached_image(
                scene["visual_prompt"],
                scene["id"],
            )

            if cached:
                cached_path = Path(cached["image_path"])

                if cached_path.is_file():
                    output.parent.mkdir(parents=True, exist_ok=True)
                    shutil.copy2(cached_path, output)
                    mark_scene_cached(scene["id"], str(output))

                    cache_hits += 1
                    success_count += 1
                    remaining -= 1

                    logging.info(
                        "CACHE HIT cảnh %s | dùng lại scene_id=%s",
                        scene["scene_no"],
                        cached["id"],
                    )
                    continue

            success = False

            for attempt in range(1, RETRY_LIMIT + 1):

                api_calls += 1

                if attempt > 1:

                    retry_count += 1
                try:
                    logging.info(
                        "Cảnh %s | lần %s/%s",
                        scene["scene_no"],
                        attempt,
                        RETRY_LIMIT,
                    )

                    generate_image(scene, output)
                    mark_scene_ready(scene["id"], str(output))
                    logging.info("Đã lưu: %s", output)

                    success = True

                    success_count += 1
                    remaining -= 1
                    time.sleep(2)
                    break

                except Exception as exc:

                    error_count += 1
                    logging.warning(
                        "Lỗi cảnh %s: %s",
                        scene["scene_no"],
                        exc,
                    )
                    if attempt < RETRY_LIMIT:
                        time.sleep(RETRY_DELAY_SECONDS * attempt)
            if not success:
                mark_scene_retry(
                    scene["id"],
                    "Không tạo được ảnh sau nhiều lần thử",
                )
                remaining -= 1

            if remaining <= 0:
                break

        finalize_case(case["id"])

    logging.info(

        "THỐNG KÊ AGENT 5 | cảnh=%s | API calls=%s | "

        "thành công=%s | lỗi=%s | retry=%s | cache=%s | lượt còn lại=%s",

        scene_count,

        api_calls,

        success_count,

        error_count,

        retry_count,
        cache_hits,

        remaining,

    )

    logging.info("Agent 5 hoàn thành")
if __name__ == "__main__":
    run()
