...
def migrate():
    with connect() as conn:
        ...
        """)
        conn.execute("CREATE INDEX IF NOT EXISTS idx_video_parts_case ON video_parts(case_id, part_no)")


def all_cases():
    with connect() as conn:
        return conn.execute("SELECT * FROM cases ORDER BY id ASC").fetchall()
...
def mark_partial(case_id: int):
    """PARTIAL tuyệt đối không giữ đường dẫn/metadata của video hoàn chỉnh."""
    with connect() as conn:
        conn.execute("""
            UPDATE cases
...
def save_part(case_id: int, part_no: int, start_scene: int, end_scene: int,
              scene_count: int, path: str, duration: float):
    with connect() as conn:
        conn.execute("""
...
def mark_part_retry(case_id: int, part_no: int, start_scene: int, end_scene: int, error: str):
    with connect() as conn:
        conn.execute("""
            ...
            ON CONFLICT(case_id, part_no) DO UPDATE SET
                status='RETRY', error=excluded.error, updated_at=CURRENT_TIMESTAMP
        """, (case_id, part_no, start_scene, end_scene,
              max(0, end_scene - start_scene + 1), error[:2000]))
        conn.execute("UPDATE cases SET video_status='RETRY', video_error=? WHERE id=?",
                     (error[:2000], case_id))


def save_final_video(case_id: int, path: str, duration: float, scene_count: int):
    with connect() as conn:
        conn.execute("""
            UPDATE cases
            ...
            WHERE id=?
        """, (path, duration, scene_count, case_id))
